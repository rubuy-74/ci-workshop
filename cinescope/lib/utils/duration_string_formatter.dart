  ...
  String toFormattedString(String singularPhrase, String pluralPhrase,
    ...
    if ((inDays / 7).floor() == 1) {
      return singularPhrase.replaceAll(
          formattingRegExp, "${(inDays / 7).floor()} week");
    }
    if ((inDays / 7).floor() > 1) {
      return pluralPhrase.replaceAll(
          formattingRegExp, "${(inDays / 7).floor()} weeks");
    }
    if ((inDays / 30).floor() == 1) {
      return singularPhrase.replaceAll(
          formattingRegExp, "${(inDays / 30).floor()} month");
    }
    return pluralPhrase.replaceAll(
        formattingRegExp, "${(inDays / 30).floor()} months");
  }
}
