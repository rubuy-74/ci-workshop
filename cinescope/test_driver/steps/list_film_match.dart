...
class ThenListFilmMatch extends Then1WithWorld<String, FlutterWorld> {
  ...
  Pattern get pattern => RegExp(
      r"it should be displayed a list of film/series cards that roughly match {string}");
  ...
  Future<void> executeStep(String input1) async {
    ...
    expect(
        (await FlutterDriverUtils.isPresent(world.driver, inputString)) ||
            (await FlutterDriverUtils.isPresent(world.driver, inputStringCaps)),
        true,
...
class ThenListFilmDoesntMatch extends Then1WithWorld<String, FlutterWorld> {
  @override
  Pattern get pattern => RegExp(
      r"it should not be displayed a card that roughly matches {string}");
