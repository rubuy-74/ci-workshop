...
class ThenSavesProfileState extends ThenWithWorld<FlutterWorld> {
  ...
  Future<void> executeStep() async {
    GlobalScenarioState().previousProfileName =
        await FlutterDriverUtils.getText(
            world.driver!, find.byValueKey("name"));
    GlobalScenarioState().previousProfileBio =
        await FlutterDriverUtils.getText(world.driver!, find.byValueKey("bio"));
  }
...
class ThenRestoresProfileState extends ThenWithWorld<FlutterWorld> {
  ...
  Future<void> executeStep() async {
    ...
    await FlutterDriverUtils.enterText(world.driver, find.byValueKey("name"),
        GlobalScenarioState().previousProfileName);
    await FlutterDriverUtils.enterText(world.driver, find.byValueKey("bio"),
        GlobalScenarioState().previousProfileBio);
    await world.driver!.scroll(
        find.byValueKey("bio"), 0, -200, const Duration(milliseconds: 500));
